...
*
*   UPDATES: last updated - 23 feb 2021 (v1.0.2)
*       v1.0.2 - Added a bunch of things (THIS VERSION IS INCOMPLETE - DO NOT UPDATE)
...
// Create a bool property with name `N`, flags `F` and value `V`
#define PBOOLPTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PBOOL, F, .value.vpbool = V}
...
// Create a int property with name `N`, flags `F` and value `V`
#define PINTPTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PINT, F, .value.vpint = {V,0,0,1}}
// Create a ranged int property within `MIN` and `MAX` with name `N`, flags `F` value `V`. 
// Pressing the spinner buttons will increase/decrease the value by `S`.
#define PINTPTR_RANGE(N, F, V, S, MIN, MAX) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PINT, F, .value.vpint = {V,MIN,MAX,S}}
...
// Create a float property with name `N`, flags `F` and value `V`
#define PFLOATPTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PFLOAT, F, .value.vpfloat = {V,0.f,0.f,1.0f,3}}
// Create a ranged float property within `MIN` and `MAX` with name `N`, flags `F` value `V` with `P` decimal digits to show. 
// Pressing the spinner buttons will increase/decrease the value by `S`.
#define PFLOATPTR_RANGE(N, F, V, S, P, MIN, MAX) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PFLOAT, F, .value.vpfloat = {V,MIN,MAX,S,P}}
...
// Create a 2D vector property with name `N`, flags `F` and the `X`, `Y` coordinates
#define PVEC2PTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PVECTOR2, F, .value.vp2 = V }
// Create a 3D vector property with name `N`, flags `F` and the `X`, `Y`, `Z` coordinates
#define PVEC3PTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PVECTOR3, F, .value.vp3 = V }
// Create a 4D vector property with name `N`, flags `F` and the `X`, `Y`, `Z`, `W` coordinates
#define PVEC4PTR(N, F, V) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_PVECTOR4, F, .value.vp4 = V }
...
// Get flags of the property at `IDX`
#define PGET_FLAGS(P, IDX) (P[IDX].flags)
// Get name of the property at `IDX`
#define PGET_NAME(P, IDX) (P[IDX].name)
// Get type of the property at `IDX`
#define PGET_TYPE(P, IDX) (P[IDX].type)
// Get value of the property at `IDX`
#define PGET_BOOL(P, IDX) (P[IDX].value.vbool)
#define PGET_BOOLPTR(P, IDX) (P[IDX].value.vpbool)
#define PGET_INT(P, IDX) (P[IDX].value.vint.val)
#define PGET_INTPTR(P, IDX) (P[IDX].value.vpint.val)
#define PGET_FLOAT(P, IDX) (P[IDX].value.vfloat.val)
#define PGET_FLOATPTR(P, IDX) (P[IDX].value.vpfloat.val)
#define PGET_V2(P, IDX) (P[IDX].value.v2)
#define PGET_V2PTR(P, IDX) (P[IDX].value.vp2)
#define PGET_V3(P, IDX) (P[IDX].value.v3)
#define PGET_V3PTR(P, IDX) (P[IDX].value.vp3)
#define PGET_V4(P, IDX) (P[IDX].value.v4)
#define PGET_V4PTR(P, IDX) (P[IDX].value.vp4)
#define PGET_RECT(P, IDX) (P[IDX].value.vrect)
#define PGET_RECTPTR(P, IDX) (P[IDX].value.vprect)
#define PGET_COLOR(P, IDX) (P[IDX].value.vcolor)
#define PGET_COLORPTR(P, IDX) (P[IDX].value.vpcolor)
#define PGET_SELECT_ACTIVE(P, IDX) (P[IDX].value.vselect.active)
// Set value of the property at `IDX`
#define PSET_BOOL(P, IDX, V) (P[IDX].value.vbool=V)
#define PSET_BOOLPTR(P, IDX, V) (P[IDX].value.vpbool=V)
#define PSET_INT(P, IDX, V) (P[IDX].value.vint.val=V)
#define PSET_INTPTR(P, IDX, V) (P[IDX].value.vpint.val=V)
#define PSET_FLOAT(P, IDX, V) (P[IDX].value.vfloat.val=V)
#define PSET_FLOATPTR(P, IDX, V) (P[IDX].value.vpfloat.val=V)
#define PSET_V2(P, IDX, V) (P[IDX].value.v2=V)
#define PSET_V2PTR(P, IDX, V) (P[IDX].value.vp2=V)
#define PSET_V3(P, IDX, V) (P[IDX].value.v3=V)
#define PSET_V3PTR(P, IDX, V) (P[IDX].value.vp3=V)
#define PSET_V4(P, IDX, V) (P[IDX].value.v4=V)
#define PSET_V4PTR(P, IDX, V) (P[IDX].value.vp4=V)
#define PSET_RECT(P, IDX, V) (P[IDX].value.vrect=V)
#define PSET_RECTPTR(P, IDX, V) (P[IDX].value.vprect=V)
#define PSET_COLOR(P, IDX, V) (P[IDX].value.vcolor=V)
#define PSET_COLORPTR(P, IDX, V) (P[IDX].value.vpcolor=V)
#define PSET_SELECT_ACTIVE(P, IDX, V) (P[IDX].value.vselect.active=V)
...
typedef struct {
    ...
    union {
        ...
        bool* vpbool;
        struct { int* val; int min; int max; int step; } vpint;
        struct { float* val; float min; float max; float step; int precision; } vpfloat;
...
void GuiDMPropertyList(Rectangle bounds, GuiDMProperty* props, int count, int* focus, int* scrollIndex) {
    ...
    const int propSlots[] = {1,1,1,2,1,3,4,5,5,5,1, 1,1,1,3,4,5,5,5};
            ...
            if(absoluteBounds.y + currentHeight + height >= bounds.y && absoluteBounds.y + currentHeight <= bounds.y + bounds.height) 
            {
                ...
                switch(props[p].type) 
                {
                    ...
                    case GUI_PROP_PBOOL: {
                        ...
                        {
                            bool val = false;
                            if(props[p].value.vpbool != NULL) val = *props[p].value.vpbool;
                            if(propState == GUI_STATE_PRESSED) val = !val; // toggle the property value when clicked
                            GuiCheckBox((Rectangle){propBounds.x+propBounds.width/2, propBounds.y + height/4, height/2, height/2}, val ? "Yes" : "No", val);
                            if(props[p].value.vpbool != NULL) *props[p].value.vpbool = val;
                        }
                    ...
                    case GUI_PROP_PINT: {
                        ...
                        int val = 0;
                        if(props[p].value.vpint.val != NULL) val = *props[p].value.vpint.val;
                        // draw property value
                        val = GuiDMSpinner((Rectangle){propBounds.x+propBounds.width/2, propBounds.y + 1, propBounds.width/2, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) - 2}, 
                                    val, props[p].value.vpint.min, props[p].value.vpint.max, props[p].value.vpint.step, 0, (propState == GUI_STATE_FOCUSED) );
                        if(props[p].value.vpint.val != NULL) *props[p].value.vpint.val = val;
                    }
                    break;
                    ...
                    case GUI_PROP_PFLOAT:
                        // draw property name
                        GuiDrawText(props[p].name, (Rectangle){propBounds.x + PROPERTY_PADDING, propBounds.y, propBounds.width/2-PROPERTY_PADDING, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)}, GUI_TEXT_ALIGN_LEFT, textColor);
                        
                        float val = 0.0f;
                        if(props[p].value.vpfloat.val != NULL) val = *props[p].value.vpfloat.val;
                        // draw property value
                        val = GuiDMSpinner((Rectangle){propBounds.x+propBounds.width/2, propBounds.y + 1, propBounds.width/2, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) - 14},
                                    val, props[p].value.vpfloat.min, props[p].value.vpfloat.max, props[p].value.vpfloat.step, props[p].value.vpfloat.precision, (propState == GUI_STATE_FOCUSED) );
                        val = GuiSlider((Rectangle){propBounds.x + PROPERTY_PADDING, propBounds.y + GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) - 11, propBounds.width-PROPERTY_PADDING, 10}, NULL, NULL, val, props[p].value.vpfloat.min, props[p].value.vpfloat.max);
                        if(props[p].value.vpfloat.val != NULL) *props[p].value.vpfloat.val = val;
                    break;
                    ...
                    case GUI_PROP_SELECT: {
                        ...
                        GuiDrawText(props[p].name, (Rectangle){propBounds.x + PROPERTY_PADDING, propBounds.y, propBounds.width/2-PROPERTY_PADDING, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)}, GUI_TEXT_ALIGN_LEFT, textColor);
                        // draw property value
                        if(CheckCollisionPointRec(GetMousePosition(), propBounds) && IsMouseButtonPressed(MOUSE_RIGHT_BUTTON)) {
                            if(props[p].value.vselect.active > 0) props[p].value.vselect.active -= 1;
                        }
                    ...
                    case GUI_PROP_VECTOR2: case GUI_PROP_VECTOR3: case GUI_PROP_VECTOR4: {
                        ...
                        else if(props[p].type == GUI_PROP_VECTOR4) fmt = TextFormat("[%.0f, %.0f, %.0f, %.0f]", props[p].value.v4.x, props[p].value.v4.y, props[p].value.v4.z, props[p].value.v4.w);
                    ...
                    case GUI_PROP_PVECTOR2: case GUI_PROP_PVECTOR3: case GUI_PROP_PVECTOR4: {
                        Rectangle titleBounds = { propBounds.x, propBounds.y, propBounds.width, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) };
                        // Collapse/Expand property on click
                        if((propState == GUI_STATE_PRESSED) && CheckCollisionPointRec(mousePos, titleBounds))
                            PROP_TOGGLE_FLAG(&props[p], GUI_PFLAG_COLLAPSED);
                        ...
                        const char* fmt = "";
                        if(props[p].type == GUI_PROP_PVECTOR2) {
                            if(props[p].value.vp2 != NULL)
                                fmt = TextFormat("[%.0f, %.0f]", (*props[p].value.vp2).x, (*props[p].value.vp2).y);
                        }
                        else if(props[p].type == GUI_PROP_PVECTOR3) {
                            if(props[p].value.vp3 != NULL)
                                fmt = TextFormat("[%.0f, %.0f, %.0f]", (*props[p].value.vp3).x, (*props[p].value.vp3).y, (*props[p].value.vp3).z);
                        }
                        else if(props[p].type == GUI_PROP_PVECTOR4) {
                            if(props[p].value.vp4 != NULL)
                                fmt = TextFormat("[%.0f, %.0f, %.0f, %.0f]", (*props[p].value.vp4).x, (*props[p].value.vp4).y, (*props[p].value.vp4).z, (*props[p].value.vp4).w);
                        }
                        ...
                        if(!PROP_CHECK_FLAG(&props[p], GUI_PFLAG_COLLAPSED)) {
                            ...
                            if(props[p].type == GUI_PROP_PVECTOR2 && props[p].value.vp2 != NULL) val.v2 = *props[p].value.vp2;
                            else if(props[p].type == GUI_PROP_PVECTOR3 && props[p].value.vp3 != NULL) val.v3 = *props[p].value.vp3;
                            else if(props[p].type == GUI_PROP_PVECTOR4 && props[p].value.vp4 != NULL) val.v4 = *props[p].value.vp4;
                            
                            Rectangle slotBounds = { propBounds.x, propBounds.y+GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)+1, propBounds.width, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)-2};
                            Rectangle lblBounds = { propBounds.x+PROPERTY_PADDING, slotBounds.y, GetTextWidth("A"), slotBounds.height};
                            Rectangle valBounds = { lblBounds.x+lblBounds.width+PROPERTY_PADDING, slotBounds.y, propBounds.width-lblBounds.width-2*PROPERTY_PADDING, slotBounds.height};
                            GuiDrawText("X", lblBounds, GUI_TEXT_ALIGN_LEFT, textColor);
                            val.v2.x = GuiDMSpinner(valBounds, val.v2.x, 0.0, 0.0, 1.0, PROPERTY_DECIMAL_DIGITS, (propState == GUI_STATE_FOCUSED) && CheckCollisionPointRec(mousePos, slotBounds) );
                            slotBounds.y += GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT);
                            lblBounds.y = valBounds.y = slotBounds.y;
                            GuiDrawText("Y", lblBounds, GUI_TEXT_ALIGN_LEFT, textColor);
                            val.v2.y = GuiDMSpinner(valBounds, val.v2.y, 0.0, 0.0, 1.0, PROPERTY_DECIMAL_DIGITS, (propState == GUI_STATE_FOCUSED) && CheckCollisionPointRec(mousePos, slotBounds) );
                            slotBounds.y += GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT);
                            lblBounds.y = valBounds.y = slotBounds.y;
                            if(props[p].value.vp2 != NULL) *props[p].value.vp2 = val.v2;
                            
                            if(props[p].type >= GUI_PROP_PVECTOR3) {
                                GuiDrawText("Z", lblBounds, GUI_TEXT_ALIGN_LEFT, textColor);
                                val.v3.z = GuiDMSpinner(valBounds, val.v3.z, 0.0, 0.0, 1.0, PROPERTY_DECIMAL_DIGITS, (propState == GUI_STATE_FOCUSED) && CheckCollisionPointRec(mousePos, slotBounds) );
                                slotBounds.y += GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT);
                                lblBounds.y = valBounds.y = slotBounds.y;
                                if(props[p].value.vp3 != NULL) *props[p].value.vp3 = val.v3;
                            }
                            
                            if(props[p].type >= GUI_PROP_PVECTOR4) {
                                GuiDrawText("W", lblBounds, GUI_TEXT_ALIGN_LEFT, textColor);
                                val.v4.w = GuiDMSpinner(valBounds, val.v4.w, 0.0, 0.0, 1.0, PROPERTY_DECIMAL_DIGITS, (propState == GUI_STATE_FOCUSED) && CheckCollisionPointRec(mousePos, slotBounds) );
                                if(props[p].value.vp4 != NULL) *props[p].value.vp4 = val.v4;
                            }
